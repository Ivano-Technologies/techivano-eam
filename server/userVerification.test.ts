...
describe('User Verification System', () => {
  ...
  describe('User Registration', () => {
    ...
    it('should store all verification fields', async () => {
      const user = await runWithTestTenantContext(() => db.getUserById(testUserId));
    ...
    it('should retrieve pending users', async () => {
      await runWithTestTenantContext(async () => {
        const pendingUser = await createUserWithPassword(
          ...
          'Pending User',
          'password123',
          { jobTitle: 'Technician', agency: 'Nigerian Red Cross Society' }
        );
        ...
        expect(pendingUsers.some(u => u.id === pendingUser.id)).toBe(true);
      });
    });
  });
